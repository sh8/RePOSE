...
def rotate_instance(img, mask, rot, rot_ini, rot_ang_min, rot_ang_max):
    h, w = img.shape[0], img.shape[1]
    degree = np.random.uniform(rot_ang_min, rot_ang_max)
    ...
    R_3d[:2, :2] = R[:2, :2]
    R_3d[2, 2] = 1
    rot_new = np.dot(R_3d, rot)
    rot_new_ini = np.dot(R_3d, rot_ini)

    return img, mask, rot_new, rot_new_ini
...
def crop_or_padding(img, mask, hcoords, hratio, wratio):
    ...
    out_img[ohbeg:ohbeg + hlen, owbeg:owbeg + wlen] = img[ihbeg:ihbeg + hlen,
                                                          iwbeg:iwbeg + wlen]
    out_mask = np.zeros([h + hd, w + wd], np.uint8)
    out_mask[ohbeg:ohbeg + hlen, owbeg:owbeg + wlen] = mask[ihbeg:ihbeg + hlen,
                                                            iwbeg:iwbeg + wlen]
...
def calc_phi(p, q):
    ...
    r_skew = skew(r)
    I_ = np.eye(3)
    out = I_ + r_skew + np.dot(r_skew, r_skew) / (1 + pq)
    return out


def calc_Rc(cc, K):
    # Calculate the principal axis vector
    pa = np.zeros(3)
    pa[2] = 1

    # Calculate the canonical object center vector
    cc = np.expand_dims(cc, axis=1)
    cc = np.dot(np.linalg.inv(K), cc)[:, 0]
    Rc = calc_phi(pa, cc)
    return Rc


def crop_or_padding_to_fixed_size_instance(img,
                                           mask,
                                           hcoords,
                                           R,
                                           t,
    ...
    hrmax = int(min(hmin + overlap_ratio * fh, h - th))
    hrmin = int(max(hmin + overlap_ratio * fh - th, 0))
    # w must > target_width else wrmax<0
    wrmax = int(min(wmin + overlap_ratio * fw, w - tw))
    ...
    Rc = calc_Rc(np.array([x_cen, y_cen, 1]), K)
    ...
    return img, mask, hcoords, R, t, K


# def crop_or_padding_to_fixed_size(
#         img, mask, amo_mask,
#         R, t, K, th, tw, overlap_ratio=0.5):
#     h, w, _ = img.shape
#     hs, ws = np.nonzero(mask)
#
#     hmin, hmax = np.min(hs), np.max(hs)
...
#     img = img[hbeg:hend, wbeg:wend]
#     mask = mask[hbeg:hend, wbeg:wend]
#     amo_mask = amo_mask[hbeg:hend, wbeg:wend]
#
#     if hpad or wpad:
#         nh, nw, _ = img.shape
#         new_img = np.zeros([th, tw, 3], dtype=img.dtype)
#         new_mask = np.zeros([th, tw], dtype=mask.dtype)
#         new_amo_mask = np.zeros([th, tw], dtype=mask.dtype)
#
#         hbeg = 0 if not hpad else (th - h) // 2
#         wbeg = 0 if not wpad else (tw - w) // 2
#
#         new_img[hbeg:hbeg + nh, wbeg:wbeg + nw] = img
#         new_mask[hbeg:hbeg + nh, wbeg:wbeg + nw] = mask
#         new_amo_mask[hbeg:hbeg + nh, wbeg:wbeg + nw] = amo_mask
#
#         img, mask, amo_mask = new_img, new_mask, amo_mask
#
#     K[0, 2] = tw / 2
#     K[1, 2] = th / 2
#
#     return img, mask, amo_mask, t
...
def mask_out_instance(img, mask, min_side=0.1, max_side=0.3):
    ...
    img[y_loc - y_side:y_loc + y_side,
        x_loc - x_side:x_loc + x_side] = np.random.randint(
            0, 255, img[y_loc - y_side:y_loc + y_side,
                        x_loc - x_side:x_loc + x_side].shape)
...
#### higher level api #####
def crop_resize_instance_v1(img,
                            mask,
                            hcoords,
                            R,
                            t,
    ...
    img, mask, hcoords, R, t, K = \
        crop_or_padding_to_fixed_size_instance(
            img, mask, hcoords,
            R, t, K, target_height, target_width,
            imheight, imwidth, overlap_ratio)

    img = cv2.resize(img, (imwidth, imheight), interpolation=cv2.INTER_LINEAR)
    mask = cv2.resize(mask, (imwidth, imheight),
...
def crop_resize_instance_v2(img,
                            mask,
                            hcoords,
                            imheight,
                            imwidth,
...
#             img, mask, hcoords, height, width, 0.8, 0.8, 1.2)
#     else:
