...
def record_occ_ann(model_meta, img_id, ann_id, images, annotations):
    ...
    inds = np.loadtxt(os.path.join('data/linemod', cls, 'test_occlusion.txt'),
                      np.str)
...
def record_real_ann(model_meta, img_id, ann_id, images, annotations):
    ...
    for ind in tqdm.tqdm(inds):
        ...
        mask_path = os.path.join(data_root, cls, 'mask',
                                 '{:04d}.png'.format(ind))
        ann_id += 1
        depth_path = os.path.join('data/linemod_orig', cls, 'data',
                                  'depth{}.dpt'.format(ind))
        anno = {
            'mask_path': mask_path,
            'depth_path': depth_path,
            'image_id': img_id,
...
def record_fuse_ann(model_meta, img_id, ann_id, images, annotations):
    ...
    for ind in tqdm.tqdm(range(250)):
        ...
        begins, poses = read_pickle(
            os.path.join(fuse_dir, '{}_info.pkl'.format(ind)))
        pose = poses[cls_idx]
        K = original_K.copy()
